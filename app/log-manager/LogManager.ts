...
const loggerNames = ["LogManager", "object-viewer.tsx", "tool-bar.tsx", "util.ts"] as const;
...
const buildLogPrefix = (backgroundColor: string, logger: string, context?: string): string[] => {
    ...
    if (context) {
        ...
        return [`%c${loggerLabel}%c %c${contextLabel}`, getStyleWhiteText(`${backgroundColor}`), ``, getStyleBlackText("lightgray")];
...
export function useLog(logger: LoggerName, context?: string): Logger {
    ...
    const createLogger = (logLevel: LogLevel, backgroundColor: string): LogSignature => {
        ...
        if (matchingLogger.level >= logLevel) {
            ...
            const logPrefixLabel: string[] = buildLogPrefix(backgroundColor, loggerLabel, contextLabel);

            const log: LogSignature =
                console.info.bind( // TODO Can also use console.???, but then you get another look in browser console.
                    console,
                    ...logPrefixLabel,
                );
    ...
    return {
        error:
            createLogger(LogLevel.ERROR, "red"),

        warning:
            createLogger(LogLevel.WARNING, "orange"),

        info:
            createLogger(LogLevel.INFO, "blue"),

        debug:
            createLogger(LogLevel.DEBUG, "green"),

        trace:
            createLogger(LogLevel.TRACE, "brown"),
    };
}
